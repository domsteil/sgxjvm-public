...
# PDF configuration
pdf_documents = [('index', u'oblivium-guide', u'Oblivium Developer Guide', u'R3')]
...
config_string_io.write('[dummy_section]\n')
with open('../../gradle.properties', 'r') as file:
    config_string_io.write(file.read())
    config_string_io.seek(0, os.SEEK_SET)
    config_parser.readfp(config_string_io)
    config = dict(config_parser.items('dummy_section'))

rst_epilog = ""
for key in config:
    rst_epilog += ".. |{key}| replace:: {value}\n".format(key = key, value = config[key])

env = dict([ (key, os.environ[key]) for key in os.environ if key.startswith('OBLIVIUM_') ])
for key in env:
    rst_epilog += ".. |{key}| replace:: {value}\n".format(key = key, value = env[key])
...
version = env['OBLIVIUM_VERSION']
# The full version, including alpha/beta/rc tags.
release = env['OBLIVIUM_VERSION']
# The version for use in the dropdown html.
html_context = {'version': env['OBLIVIUM_VERSION']}
