...
JVM-in-SGX projects are built using the popular Gradle tool. The build process for an enclave is complex and involves
...
you want to use in your ``settings.gradle`` file. Kotlin is not required to use JVM-in-SGX, but the RNG enclave does use it.
...
JVM-in-SGX will load and is analogous to the ``Main-Class`` attribute executable JARs normally use. We make the test
...
The ``EnclaveApi`` provides access to functionality provided by JVM-in-SGX. Inside the enclave many ordinary Java APIs
...
.. note:: The JVM-in-SGX team always endeavours to provide SGX compatible implementations of the standard Java API whenever
